...
export const BookList = ({ books, onPressBook }: BookListProps) => {
  return (
    books && (
      <View className="w-[calc(100%-1rem)] h-fit mx-2 p-2">
        <View className="flex flex-col gap-4">
          {books.map((book) => (
            <View>
            <BookCard book={book} onPressBook={onPressBook} />
            </View>
          ))}
        </View>
      </View>
...
export const BookCard = ({ book, onPressBook }: BookCardProps) => {
  const progress = ((book.currentPage ?? 0) / book.numberOfPages) * 100;

  return (
    <TouchableOpacity key={book.id} onPress={() => onPressBook(book)}>
